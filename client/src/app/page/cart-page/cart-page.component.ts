...
export class CartPageComponent implements OnInit {
  ...
  oSub: Subscription;
  netWorth: number = 0;
  netWorthProducts$: BehaviorSubject<number> = new BehaviorSubject<number>(
    this.cartService.getNetWorthCart()
  );
  ...
  private getNetWorthCart() {
    return this.itemsCart.reduce(
      (acc, item) => acc + parseInt(String(item.price * item.counter)),
      0
    );
  }
